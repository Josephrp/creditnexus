...
def upgrade() -> None:
    ...
    # Create document_signatures table if it doesn't exist
    if 'document_signatures' not in existing_tables:
        ...
        if 'ix_document_signatures_document_id' not in existing_indexes:
            try:
                op.create_index('ix_document_signatures_document_id', 'document_signatures', ['document_id'])
            except:
                pass
        if 'ix_document_signatures_generated_document_id' not in existing_indexes:
            try:
                op.create_index('ix_document_signatures_generated_document_id', 'document_signatures', ['generated_document_id'])
            except:
                pass
        if 'ix_document_signatures_signature_request_id' not in existing_indexes:
            try:
                op.create_index('ix_document_signatures_signature_request_id', 'document_signatures', ['signature_request_id'], unique=True)
            except:
                pass
        if 'ix_document_signatures_signature_status' not in existing_indexes:
            try:
                op.create_index('ix_document_signatures_signature_status', 'document_signatures', ['signature_status'])
            except:
                pass
    
    # Create document_filings table if it doesn't exist
    if 'document_filings' not in existing_tables:
        op.create_table(
            'document_filings',
            sa.Column('id', sa.Integer(), nullable=False),
            sa.Column('document_id', sa.Integer(), nullable=False),
        ...
        # Table exists, but ensure indexes exist
        existing_indexes = [idx['name'] for idx in inspector.get_indexes('document_filings')]
        if 'ix_document_filings_document_id' not in existing_indexes:
            try:
                op.create_index('ix_document_filings_document_id', 'document_filings', ['document_id'])
            except:
                pass
        if 'ix_document_filings_generated_document_id' not in existing_indexes:
            try:
                op.create_index('ix_document_filings_generated_document_id', 'document_filings', ['generated_document_id'])
            except:
                pass
        if 'ix_document_filings_deal_id' not in existing_indexes:
            try:
                op.create_index('ix_document_filings_deal_id', 'document_filings', ['deal_id'])
            except:
                pass
        if 'ix_document_filings_agreement_type' not in existing_indexes:
            try:
                op.create_index('ix_document_filings_agreement_type', 'document_filings', ['agreement_type'])
            except:
                pass
        if 'ix_document_filings_jurisdiction' not in existing_indexes:
            try:
                op.create_index('ix_document_filings_jurisdiction', 'document_filings', ['jurisdiction'])
            except:
                pass
        if 'ix_document_filings_filing_status' not in existing_indexes:
            try:
                op.create_index('ix_document_filings_filing_status', 'document_filings', ['filing_status'])
            except:
                pass
        if 'ix_document_filings_deadline' not in existing_indexes:
            try:
                op.create_index('ix_document_filings_deadline', 'document_filings', ['deadline'])
            except:
                pass
        if 'ix_document_filings_filing_reference' not in existing_indexes:
            try:
                op.create_index('ix_document_filings_filing_reference', 'document_filings', ['filing_reference'], unique=True)
            except:
                pass
